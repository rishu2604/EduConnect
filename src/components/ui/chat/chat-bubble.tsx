...
const ChatBubbleAvatar: React.FC<ChatBubbleAvatarProps> = ({ src, fallback, className }) => (
  <Avatar className={className}>
    ...
    <AvatarFallback className='font-semibold text-stone-700'>{fallback}</AvatarFallback>
  </Avatar>
);
...
const chatBubbleMessageVariants = cva('p-4', {
  variants: {
    variant: {
      received: 'bg-gray-300 text-secondary-foreground rounded-r-lg rounded-tl-lg',
      sent: 'bg-gray-700 text-primary-foreground rounded-l-lg rounded-tr-lg'
    },
    layout: {
      default: '',
...
const ChatBubbleActionWrapper = React.forwardRef<HTMLDivElement, ChatBubbleActionWrapperProps>(
  ({ variant, className, children, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
        'opacity-1 absolute top-1/2 flex -translate-y-1/2 transition-opacity duration-200 group-hover:opacity-100',
        variant === 'sent'
